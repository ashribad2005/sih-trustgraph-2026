...
class DashboardMetricsView(APIView):
    ...
    def get(self, request):
        ...
        # Active alerts = OPEN + UNDER_REVIEW cases
        active_cases = FraudCase.objects.filter(
            status__in=[FraudCase.StatusChoices.OPEN, FraudCase.StatusChoices.UNDER_REVIEW]
        )
        active_alerts_count = active_cases.count()

        # Protected value = total amount associated with active high-risk cases
        protected_val = active_cases.aggregate(
            Sum('transaction__amount')
        )['transaction__amount__sum'] or 0
        ...
        return Response({
            "total_monitored_volume": float(total_vol),
            "total_screened_transactions": total_screened,
            "active_alerts_count": active_alerts_count,
            "protected_value": float(protected_val),
            "blockchain_anchored_count": anchored_count,
        })
